...
export default function Login() {
  ...
  async function handleSignUp(e: FormEvent) {
    ...
    // Check if email is on the allowed list (server-side; anon reads of
    // dhub.allowed_users were removed 2026-06-11)
    let allowed = false
    try {
      const r = await fetch(`/api/auth/check-allowed?email=${encodeURIComponent(email.toLowerCase())}`)
      allowed = (await r.json()).allowed === true
    } catch {
      allowed = false
    }
